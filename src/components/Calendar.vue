...
let endOfMonth = new Date(current.getFullYear(), current.getMonth() + 1, 0);
  ...
  for (let i = startOfMonth.getDate(); i < endOfMonth.getDate(); i++) {
    ...
    console.log(JSON.stringify(current.toLocaleDateString('en-ZA', { weekday:"long", month:"long", day:"numeric"})))
    console.log(current.toLocaleDateString('en-ZA', { weekday:"short"}));
    current.setDate(current.getDate() + 1);
  }
  //return week;
//}
let now = new Date().toLocaleDateString('en-ZA', { weekday:"long"});
console.log(now)
